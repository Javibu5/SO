...
int main() 
{    
    pthread_t thd1, thd2; //Declaración de dos hebras, hilos o procesos ligeros. NO CREACION 
    ...
    pthread_create (&thd1, NULL, hiloMensaje, (void *) &param1); 
    pthread_create (&thd2, NULL, hiloMensaje, (void *) &param2);
    
    /*Esperamos la finalización de los hilos. Si la función devolviera algo habría que recogerlo con el segundo argumento, que en este caso esta a NULL. Cuando el segundo argumento no es NULL, se recogen los resultados que vienen de pthread_exit(), que se explicará a continuación.*/
    /*Si no se ponen estos join() en el programa principal y simplemente lanzamos los dos hilos y finalizamos, lo más probable es que los hilos no lleguen a ejecutarse completamente o incluso que no lleguen ni a terminar de arrancar antes de que el programa principal termine.*/
